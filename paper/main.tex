...
live language models: a capability ladder of local models on a frozen 100-item decision task obeys
$\Delta G \propto \I(X;Y)$ (Pearson $r=0.996$), exhibits over-confidence as measurable dissipation that shrinks
with capability, and confirms diversity-beats-redundancy; the fleet-pricing claim is reported with its honest
boundary (no volatility-harvesting ``demon'' on a positively-correlated ladder, but pricing wins under a compute
budget). None of the underlying mechanisms is individually new; the contribution is their unification under one
...
and realized growth $\Delta G_a=\text{mean}\,\ln\!\big(p_a(x\mid y_a)/r(x)\big)$. The agents are a weak$\to$strong
ladder of local models ($1.5$B, $3$B, $7$B parameters), greedy decoding. Posteriors are calibrated on a
$48$-item fit split and scored on a $52$-item holdout; in-sample, the oracle posterior with $r=q$ makes
$\Delta G=\I$ an arithmetic identity (used only to confirm the units), so the empirical content is the ladder
monotonicity, the out-of-sample tracking, and the fleet result.

\paragraph{R1: the bridge holds and rises with scale.} Mutual information increases monotonically along the
ladder and out-of-sample value-growth tracks it almost perfectly (Table~\ref{tab:r1}; Pearson $r=0.996$
between $\I$ and $\Delta G_{\text{holdout}}$). Value-throughput is information-throughput, on real behavior.
...
\bottomrule
\end{tabular}
\caption{R1 (nats). $\I(X;Y)$ rises with capability; $\Delta G_{\text{hold}}$ tracks it ($r=0.996$). The world
entropy is $H(X)=1.92$ nats, so model C captures $93\%$ of it, model A $66\%$.}
\label{tab:r1}
\end{table}
...
(Table~\ref{tab:r2}). It is large for the weak model and shrinks with scale --- a quantitative statement that
weaker agents must be more humble. For model A, confident error drives realized growth \emph{negative}, exactly
the Second Law's prediction \eqref{eq:secondlaw}.

\begin{table}[h]\centering
\begin{tabular}{lccc}
\toprule
model & $G_{\text{cal}}$ & $G_{\text{over}}$ & dissipated \\
\midrule
A (1.5B) & $+0.915$ & $-3.255$ & 4.17 \\
B (3B)   & $+1.242$ & $-0.863$ & 2.11 \\
C (7B)   & $+1.423$ & $+0.731$ & 0.69 \\
\bottomrule
\end{tabular}
\caption{R2 (nats). Dissipation from false certainty falls as capability rises; the weakest agent's
over-confidence destroys value outright.}
\label{tab:r2}
\end{table}

\paragraph{R3: value per joule.} Mutual information \emph{per second of compute} falls with scale
($0.74\to0.38\to0.30$ nats/s), as does $\I$ per billion parameters ($0.85\to0.52\to0.25$). The weakest model is
the most efficient perceiver per unit compute --- the value-theoretic reason a cheap reflex is worth running on
a narrow task. (This is the $\I$/compute curve across model scale, not a within-model prompt ablation; see
\S\ref{sec:limits}.)

\paragraph{R4: diversity beats redundancy.} Two different models jointly perceive more of $H(X)$ than either
alone; an identical greedy re-run adds exactly zero. The best diverse pair (B+C) reaches $\I=1.869$ nats, $+0.090$
over C alone and closing most of the gap to $H(X)=1.921$. This is the fleet-ceiling prediction \eqref{eq:ceiling}
on real agents.

\paragraph{R5: pricing beats pooling, but only where there is diversity to price (reported honestly).} On raw
out-of-sample growth, the Kelly/price fleet ($+1.343$) beats the equal-weight ensemble ($+1.324$) but
\emph{does not beat the single best model} ($+1.423$). There is no Shannon's demon here, and the theory says
why: a capability ladder on the \emph{same} task yields positively-correlated agents (R4 quantifies the residual
...
yields $0.531$ nats/s against C's $0.243$, and a budget-aware price $\propto \I_a/\text{cost}_a$ (the shadow
price $\lambda=K/E$ of \S\ref{sec:crossframe}) achieves $0.393$ nats/s, beating the best single model's density.
Pricing therefore pays exactly where \S\ref{sec:fleet} says it should: as the lever that chooses the operating
point under a resource constraint, not as a free lunch that beats the best agent when compute is unlimited. The
demon needs \emph{perception diversity} --- different slices of $H(X)$ --- which a same-task scale ladder lacks
by construction. We mark this as a falsifiable boundary, not a defeat.
...
ladder monotonicity, the out-of-sample tracking, and R5 are empirical; R3 is an $\I$/compute curve across model
