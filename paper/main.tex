...
by the agent's goal. From the compounding dynamics of a multiplicatively reinvested resource we derive a
logarithmic measure of value, $V=\sum_i k_i\ln e_i$ --- a form that scale invariance independently forces via
a Cauchy functional equation, confirming it from a second route. From the same compounding dynamics we derive a
...
realizes. Two independent routes force the same functional form; we present both.

\paragraph{Primary route: compounding.}
The decisive constraint is that value compounds: resource created this round is reinvested next round. For any
multiplicatively-compounding resource, the correct measure of long-run gain is the expected log of the
multiplier, not its arithmetic mean --- the ergodicity argument of \citet{peters2019}, and independently the
basis of Kelly's criterion \citep{kelly1956}. Applied channel by channel: if channel $i$ receives resource $e_i$
and advances the agent's goal with weight $k_i$, the contribution to the time-average growth rate of the
reinvested resource is $k_i\ln e_i$. Summing over independent channels gives
\begin{equation}\label{eq:Vcomp}
V(e) \;=\; \sum_{i=1}^K k_i\ln e_i.
\end{equation}
The logarithm arises from the geometry of compounding: it is the unique functional form whose expectation equals
the time-average of a multiplicative process (\S\ref{sec:capacity} makes this explicit via the growth-rate
program). \emph{No scale assumption is required.}

\paragraph{Cross-check via scale invariance.} Three axioms, independently natural, recover the same form and
supply a rigorous existence-uniqueness proof.
...
Axiom~\ref{ax:scale} is independently motivated (value has no privileged zero of scale, just as information has
no privileged base of logarithm), and is in fact a \emph{consequence} of the compounding derivation: one can
verify directly that $k_i\ln(\lambda e_i)=k_i\ln e_i+k_i\ln\lambda$, so the logarithm satisfies it exactly.
The axiomatic route therefore cross-checks rather than founds the measure.
...
\emph{focus is worth $K\,H(\hat k)$ nats of value}. Second, the logarithm is forced by compounding (primary
route above and \S\ref{sec:capacity}); Axiom~\ref{ax:scale} forces it a second time via the Cauchy equation
(Appendix~\ref{app:log}). A form over-determined by two independent derivations is the signature of a real
structural law. The Appendix proof uses the axiomatic route; the compounding route does not invoke scale
invariance.
...
Weber--Fechner. We do not claim the form as novel; our departure is to \emph{derive} it from multiplicative compounding on a
physical resource \citep{peters2019,kelly1956} --- scale invariance then appears as a consequence and
independent cross-check rather than the foundation --- and to ground it in a conserved scarce resource (free
energy being the natural physical candidate) supplying a unit and a cross-frame exchange rate that expected
utility lacks.
...
and the empirical scope is three task domains and a ten-model, five-family ladder (\S\ref{sec:v2}) --- evidence
the bridge \emph{generalizes across these}, not yet a universal law, and not independently replicated.
...
\emph{This appendix gives the scale-invariance (axiomatic) route to $V=\sum_i k_i\ln e_i$, which
cross-checks the compounding derivation of \S\ref{sec:measure}. The two routes are independent: neither
invokes the other.}
