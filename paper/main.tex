...
laws generalize. A fleet-pricing experiment is reported scoped and primary-metric-first: value-pricing recovers
...
ladder, so cost-aware pricing reduces to quality-first routing and selects the identical model. Only on a
\emph{secondary} latency budget --- where compute cost varies $5$--$6\times$, a real cost gradient --- does
value-price beat a cost-\emph{blind} router; and even there it \emph{loses} to a hand-tuned router that itself
prices cost (accuracy/cost). Against the naive baselines (round-robin, equal-weight) value-price wins on both
metrics, so priced routing is no worse than ad-hoc --- but that is the floor, not the claim.

\begin{quote}
Value-pricing derives cost-aware routing from first principles --- it ties good hand-tuned routing under a token
budget, beats a cost-blind router under a latency budget, and does not outperform a cost-aware engineer. Its
