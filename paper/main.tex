...
tracks realized \emph{capability} rather than parameter count (monotonically across all four; a cross-family
model that is larger yet weaker lands at correspondingly lower $\I$), out-of-sample $\Delta G$ tracks $\I(X;Y)$
across every model,
...
correlated agents, but pricing wins under a compute budget). The laws hold in the smooth, concave
(diminishing-returns) regime; threshold, satiation, and risk-seeking goals lie outside it. None of the
underlying mechanisms is individually new; the contribution is their unification under one
...
routes is the signature of a real structural law, not a modeling convenience.
...
a physical resource, to ground it in a conserved scarce resource (free energy being the natural physical candidate) supplying a unit
and a cross-frame exchange rate that expected utility lacks, and to show it is forced a second time by
compounding \citep{peters2019}.
...
\paragraph{The decisive test.} The present evidence shows the laws are well-defined and hold on one task; it
does not establish them as general. The experiment that would do so --- or falsify the framework --- is direct:
on a fleet of \emph{heterogeneous, resource-constrained} agents (specialists perceiving different slices of
$X$, under a real compute budget), does value-pricing measurably beat a strong hand-tuned orchestration
baseline, and does collective throughput obey the predicted ceiling $\sum_a G_a\le H(X)$ quantitatively? A
negative there --- pricing failing to beat the baseline even under cost and diversity, or throughput exceeding
the entropy ceiling --- would refute the governance claim at its center. We have shown the bridge holds; this is
the experiment that would show it \emph{matters}.
